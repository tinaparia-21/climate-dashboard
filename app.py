...
def get_feedback_worksheet():
    ...
    Credentials are read from Streamlit secrets (st.secrets), never from a
    committed JSON file. Returns None if secrets aren't configured yet,
    so the app can fall back to local CSV storage.
    """
    ...
    with st.form(key="quiz_feedback_form"):
        ...
        if submit_feedback:
            ...
            if worksheet is not None:
                # --- PRIMARY PATH: Google Sheets (persists across redeploys) ---
                try:
                    existing = worksheet.get_all_values()
                    rows = existing if existing else [SHEET_HEADER]
                    user_found = False
                    for i, row in enumerate(rows[1:], start=2):  # sheet rows are 1-indexed, row 1 = header
                        if len(row) >= 2 and row[1] == session_user_name:
                            worksheet.update(
                                f"A{i}:E{i}",
                                [[row[0], session_user_name, score_display, submission_date, user_feedback]],
                            )
                            user_found = True
                            break
                    if not user_found:
                        next_id = len(rows)  # header counts as row 0 in our id scheme
                ...
                # --- FALLBACK PATH: local CSV (only persists until the app restarts) ---
                import csv
                import os
                ...
                st.success(f"🎉 Thank you, {session_user_name}! Feedback saved locally (set up Google Sheets secrets for persistent storage).")
